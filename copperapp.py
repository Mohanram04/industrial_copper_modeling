...
def predict_status(ctry,itmtp,aplcn,wth,prdrf,qtlg,cstlg,tknslg,slgplg,itmdt,itmmn,itmyr,deldtdy,deldtmn,deldtyr):
    ...
    with open("F:/copper_moddeling/New folder/Classification_model.pkl","rb") as f:
...
def predict_selling_price(ctry,sts,itmtp,aplcn,wth,prdrf,qtlg,cstlg,
    ...
    dydy= int(deldtyr)
    #modelfile of the classification
    with open("F:/copper_moddeling/New folder/Regression_model.pkl","rb") as r1:
...
with st.sidebar:
    option = option_menu('MOHAN', options=["PREDICT SELLING PRICE", "PREDICT STATUS"])
